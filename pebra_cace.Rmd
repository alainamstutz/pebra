...
df_or <- df_cace %>% # remove ARM and ENDPOINT and redundant variables (duplicates, used in overall categories, etc.)
...
# adjusted odds ratio model for each variable
model <- glm(complier ~ 
               Gender 
             + `Age at enrolment` 
             ...
             + `How do you believe you were infected with HIV?`
               , data = df_or, family = "binomial")

summary(model)

# Create a table with absolute values
abs_table <- table(df_or$Gender, df_or$complier)
# Add percentages using prop.table()
perc_table <- prop.table(abs_table, margin = 2) * 100
# Combine the absolute and percentage values in the same cell
combined_table <- abs_table
combined_table[] <- paste0(abs_table, " (", round(perc_table, 1), "%)")
# Exclude the unnecessary levels from the combined table
combined_table <- combined_table[-which(rownames(combined_table) == "female"), ]

# Create an empty data frame to store the results
results_or <- NULL
results_or <- data.frame(Variable = c("Male"))
results_or$`Refused PEBRA` <- combined_table[1]
results_or$`Accepted PEBRA` <- combined_table[2]
results_or$OddsRatio <- exp(coef(summary(model))[2, "Estimate"])
results_or$CI_Lower <- exp(confint(model)[2,1])
results_or$CI_Upper <- exp(confint(model)[2,2])

# Print the baseline table
# knitr::kable(baseline_table, caption = "Baseline characteristics predicting acceptance of PEBRA and their odds ratios with 96% confidence intervals, adjusted for all other variables")
...
sjPlot::tab_model(fit.2sls)
sjPlot::tab_model(primary)
